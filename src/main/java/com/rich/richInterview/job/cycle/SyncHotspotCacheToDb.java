...
public class SyncHotspotCacheToDb {
     ...
     * 题库热点缓存前缀
     */
    private static final String BANK_HOTSPOT_CACHE_PREFIX = "question_bank_hotspot:";
    ...
    public void syncHotspotCacheToDatabase() {
        ...
        try {
            // 获取题目热点与题库热点缓存key
            Set<String> questionCacheKeys = redisTemplate.keys(HOTSPOT_CACHE_PREFIX + "*");
            Set<String> bankCacheKeys = redisTemplate.keys(BANK_HOTSPOT_CACHE_PREFIX + "*");

            if (CollUtil.isEmpty(questionCacheKeys) && CollUtil.isEmpty(bankCacheKeys)) {
                log.info("没有找到需要同步的热点缓存数据");
                return;
            }

            if (!CollUtil.isEmpty(questionCacheKeys)) {
                log.info("题目热点：发现 {} 个缓存key需要同步", questionCacheKeys.size());
                int qSuccess = 0;
                int qFail = 0;
                for (String cacheKey : questionCacheKeys) {
                    try {
                        ...
                        log.error("同步题目热点缓存key: {} 时发生异常", cacheKey, e);
                        qFail++;
                    }
                }
                log.info("题目热点同步完成，成功: {}, 失败: {}", qSuccess, qFail);
            }

            if (!CollUtil.isEmpty(bankCacheKeys)) {
                log.info("题库热点：发现 {} 个缓存key需要同步", bankCacheKeys.size());
                int bSuccess = 0;
                int bFail = 0;
                for (String cacheKey : bankCacheKeys) {
                    try {
                        if (syncSingleBankHotspotCache(cacheKey)) {
                            bSuccess++;
                        } else {
                            bFail++;
                        }
                    } catch (Exception e) {
                        log.error("同步题库热点缓存key: {} 时发生异常", cacheKey, e);
                        bFail++;
                    }
                }
                log.info("题库热点同步完成，成功: {}, 失败: {}", bSuccess, bFail);
            }
    ...
    private boolean syncSingleHotspotCache(String cacheKey) {
        try {
            ...
            log.error("同步单个题目热点缓存数据时发生异常: {}", cacheKey, e);
            return false;
        }
    }
     ...
     * @return 是否同步成功
     */
    private boolean syncSingleBankHotspotCache(String cacheKey) {
        try {
            // 解析缓存key获取questionBankId和字段名
            // 格式: question_bank_hotspot:questionBankId:fieldName
            String[] keyParts = cacheKey.split(":");
            if (keyParts.length != 3) {
                log.warn("缓存key格式不正确: {}", cacheKey);
                return false;
            ...
            Long questionBankId = Long.parseLong(keyParts[1]);
            String fieldName = keyParts[2];
            ...
            // 获取数据库中的当前值
            QuestionBankHotspot dbHotspot = questionBankHotspotService.getByQuestionBankId(questionBankId);
            if (dbHotspot == null) {
                log.warn("数据库中不存在questionBankId: {} 的题库热点数据", questionBankId);
                return false;
            }

            // 获取数据库中对应字段的当前值
            Integer dbValue = getBankFieldValue(dbHotspot, fieldName);
            if (dbValue == null) {
                log.warn("无法获取数据库中字段 {} 的值", fieldName);
                return false;
            }
            ...
            // 如果缓存值大于数据库值，说明有新的增量需要同步
            if (currentValue > dbValue) {
                boolean updateResult = updateBankDatabaseField(questionBankId, fieldName, currentValue);
                if (updateResult) {
                    log.debug("成功同步题库缓存数据到数据库: questionBankId={}, field={}, value={}",
                            questionBankId, fieldName, currentValue);
                    return true;
                } else {
                    log.warn("同步题库缓存数据到数据库失败: questionBankId={}, field={}, value={}",
                            questionBankId, fieldName, currentValue);
                    return false;
                }
            } else {
                log.debug("缓存值({})不大于数据库值({})，无需同步: questionBankId={}, field={}",
                        currentValue, dbValue, questionBankId, fieldName);
                return true;
            }

        } catch (Exception e) {
            log.error("同步单个题库热点缓存数据时发生异常: {}", cacheKey, e);
            return false;
        }
    }
     ...
     * 获取题库热点对象中指定字段的值
     */
    private Integer getBankFieldValue(QuestionBankHotspot hotspot, String fieldName) {
        switch (fieldName) {
            case "viewNum":
                return hotspot.getViewNum();
            case "starNum":
     ...
     * @return 是否更新成功
     */
    private boolean updateBankDatabaseField(Long questionBankId, String fieldName, Integer newValue) {
        try {
            // 获取当前题库热点数据
            QuestionBankHotspot hotspot = questionBankHotspotService.getByQuestionBankId(questionBankId);
            if (hotspot == null) {
                return false;
            }
            ...
            // 更新数据库
            return questionBankHotspotService.updateById(hotspot);

        } catch (Exception e) {
            log.error("更新题库数据库字段时发生异常: questionBankId={}, field={}, value={}",
                    questionBankId, fieldName, newValue, e);
            return false;
        }
    }
}